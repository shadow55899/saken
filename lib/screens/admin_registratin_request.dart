import 'package:apartment/models/requests_model.dart';
import 'package:apartment/widgets/boxInformation.dart';
import 'package:apartment/widgets/photo.dart';
...
class AdminRegistratinRequest extends StatelessWidget {
  ...
  void showDialog(BuildContext context, Userg user) {
    ...
    Get.defaultDialog(
      ...
      textConfirm: 'Accept',
      onCancel: () async {
        bool success = await controller.rejectRequest(user.id); // call API
        if (success) {
          print("in screen success");
          controller.RequestList.remove(user); // remove only if API succeeds
